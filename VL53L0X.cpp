...
bool VL53L0X::init(bool ioMode2v8) {
    ...
    if (ioMode2v8) {
        // set bit 0
        this->writeRegister(VHV_CONFIG_PAD_SCL_SDA__EXTSUP_HV,
                            this->readRegister(VHV_CONFIG_PAD_SCL_SDA__EXTSUP_HV) | 0x01);
    }
...
void VL53L0X::writeRegister(uint8_t reg, uint8_t value) {
    ...
    _wire->write((uint8_t) reg, (uint8_t) value);
...
void VL53L0X::writeRegister16Bit(uint8_t reg, uint16_t value) {
    ...
    _wire->beginTransmission(address);
    _wire->write16((uint8_t) reg, (uint16_t) valueFixed);
    _wire->endTransmission();
}
...
void VL53L0X::writeRegisterMultiple(uint8_t reg, const uint8_t *source, uint8_t count) {
    ...
    _wire->beginTransmission(address);
    _wire->writeBlock((uint8_t) reg, buffer, count);
    _wire->endTransmission();
    delete[] buffer;
}
...
uint32_t VL53L0X::readRegister32Bit(uint8_t reg) {
    ...
    _wire->beginTransmission(address);
    int p = _wire->readBlock(reg, buffer, 4);
    _wire->endTransmission();
...
void VL53L0X::readRegisterMultiple(uint8_t reg, uint8_t *destination, uint8_t count) {
    ...
    _wire->beginTransmission(address);
    int p = _wire->readBlock(reg, buffer, count);
    _wire->endTransmission();
...
bool VL53L0X::setMeasurementTimingBudget(uint32_t budgetMicroseconds) {
    ...
    uint16_t finalRangeTimeoutMCLKs = timeoutMicrosecondsToMclks(finalRangeTimeoutMicroseconds,
...
void
VL53L0X::getSequenceStepTimeouts(const VL53L0X::SequenceStepEnables *enables, VL53L0X::SequenceStepTimeouts *timeouts) {
    ...
    timeouts->msrcDssTccMicroseconds = this->timeoutMclksToMicroseconds(timeouts->msrcDssTccMCLKs,
    ...
    timeouts->preRangeMicroseconds = this->timeoutMclksToMicroseconds(timeouts->preRangeMCLKs,
                                                                      timeouts->preRangeVCSELPeriodPCLKs);
    ...
    timeouts->finalRangeMicroseconds = this->timeoutMclksToMicroseconds(timeouts->finalRangeMCLKs,
                                                                        timeouts->finalRangeVCSELPeriodPCLKs);
}
...
uint16_t VL53L0X::encodeTimeout(uint16_t timeoutMCLKs) {
    ...
    lsByte = static_cast<uint32_t>(timeoutMCLKs - 1);
